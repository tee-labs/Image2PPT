...
- Supports PDF input: reads the PDF text layer directly, skips OCR, and reproduces text with 100% fidelity (see below).
...
Or pass a PDF — each PDF page maps to one PPT page:

```bash
python scripts/convert.py --source /path/to/deck.pdf
# Tune the raster DPI (default 300)
python scripts/convert.py --source /path/to/deck.pdf --pdf-dpi 200
```

In PDF mode, PyMuPDF reads the PDF text layer directly (exact Unicode + per-character bboxes, confidence=1.0), so OCR is skipped entirely and the text matches the source 100%. Each page is also rasterised to PNG so the existing erase / inventory / calibration stages can reuse the visual reference. For purely scanned PDFs (no text layer), export the pages to images first and use the image-folder path instead.
...
- `--pdf-dpi 300`: raster DPI for PDF input (default 300). Drop to 200 for very long decks or when output size matters.
...
│   ├── ocr/          # OCR, cross-verification, review application, native PDF text ingest
...
- The one-command entry accepts a single image, an image folder, or a PDF file. Image inputs not named `page_NN.<ext>` are copied into the run directory and numbered automatically; PDF inputs are rasterised and text-extracted by `scripts/ocr/pdf_ingest.py`.
